...
export const registerLogsCommand = (program: Command): void => {
  program
    ...
    .action((options: LogsOptions) => {
      ...
      const outputRecords = options.review === true
        ? records.filter((record) => ["deny", "ask", "redact"].includes(record.decision.effect))
        : records;
      ...
      console.log(options.review === true ? renderAuditReviewMarkdown(records) : renderAuditMarkdown(records));
    });
};
